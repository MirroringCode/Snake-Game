...
function init() {
	...
	snake = new Snake({x: tileSize * Math.floor(width / (2 * tileSize)), y: tileSize * Math.floor(height / (2 * tileSize)) }, "#ffff00");
...
// We're treating the food as an object.
...
window.addEventListener('keydown', function (evt) {
	...
	evt.preventDefault();
	if (snake.velY != 1 && snake.x >= 0 
		&& snake.x <= width && snake.y >= 0 && snake.y <= height)
		snake.dir(0, -1);
	}
